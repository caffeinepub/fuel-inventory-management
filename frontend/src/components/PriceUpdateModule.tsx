...
import { useGetCurrentPrices, useGetPriceHistory, useUpdatePrice, useIsCallerAdmin } from '../hooks/useQueries';
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">Manage fuel prices and view price history</p>
      </div>
          ...
          <CardContent>
            <div className="space-y-3">
              {currentPrices.map(([type, price]) => (
                <div key={type} className="flex justify-between items-center p-4 border rounded-lg">
                  ...
                  <span className="text-2xl font-bold">₹{price.toFixed(2)}/L</span>
                </div>
              ))}
            </div>
          </CardContent>
      ...
      <Card>
        ...
        <CardContent>
          <Table>
            ...
            <TableBody>
              {priceHistory.slice().reverse().slice(0, 20).map((update, index) => (
                <TableRow key={index}>
                  <TableCell>{formatTimestamp(update.effectiveDate)}</TableCell>
                  <TableCell>
                    {update.fuelType === 'petrol' ? 'Petrol' : 'Diesel'}
                  </TableCell>
                  <TableCell className="text-right font-semibold">₹{update.newPrice.toFixed(2)}/L</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
