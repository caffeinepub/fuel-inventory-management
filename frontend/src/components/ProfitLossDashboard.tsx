  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">Financial overview and performance metrics</p>
      </div>
              ...
              <div className="p-4 bg-green-50 dark:bg-green-950 rounded-lg">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="text-lg font-bold text-green-600">
                    ₹{sales.filter((s) => s.fuelType === 'petrol').reduce((sum, s) => sum + s.total, 0).toFixed(2)}
                  </span>
                </div>
                <p className="text-xs text-muted-foreground">
                  {sales.filter((s) => s.fuelType === 'petrol').length} transactions
                </p>
              </div>
              ...
              <div className="p-4 bg-blue-50 dark:bg-blue-950 rounded-lg">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="text-lg font-bold text-blue-600">
                    ₹{sales.filter((s) => s.fuelType === 'diesel').reduce((sum, s) => sum + s.total, 0).toFixed(2)}
                  </span>
                </div>
                <p className="text-xs text-muted-foreground">
                  {sales.filter((s) => s.fuelType === 'diesel').length} transactions
                </p>
              </div>
            </div>
          </CardContent>
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-3">
              {['maintenance', 'electricity', 'salaries', 'supplies'].map((cat) => {
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-3">
            ...
            <div className={`flex justify-between items-center p-4 rounded-lg ${netProfit >= 0 ? 'bg-green-50 dark:bg-green-950' : 'bg-red-50 dark:bg-red-950'}`}>
              <span className="font-bold">Net {netProfit >= 0 ? 'Profit' : 'Loss'}</span>
