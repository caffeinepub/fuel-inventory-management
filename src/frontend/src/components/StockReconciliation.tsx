import { useGetTanks, useGetSales } from '../hooks/useQueries';
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">Daily stock comparison and variance analysis</p>
      </div>
      ...
      <Card>
        ...
        <CardContent>
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="text-right">Calculated Closing (L)</TableHead>
            ...
            <TableBody>
              {reconciliation.map((row) => (
                <TableRow key={row.tankId}>
                  ...
                  <TableCell className="text-right">{row.calculatedClosing.toFixed(2)}</TableCell>
                  <TableCell className="text-right">{row.actualClosing.toFixed(2)}</TableCell>
                  <TableCell className="text-right">
                    {Math.abs(row.variance) > 0.1 ? (
                      ...
                      <span className="text-muted-foreground">{row.variance.toFixed(2)}</span>
                    )}
                  </TableCell>
                </TableRow>
              ))}
          ...
          {reconciliation.length === 0 && (
            <p className="text-center text-muted-foreground py-8">No tanks to reconcile</p>
          )}
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-3">
              ...
              .map((row) => (
                <div
                  ...
                  className={`p-3 rounded-lg border ${
                    row.variance < 0 ? 'border-destructive/50 bg-destructive/5' : 'border-primary/50 bg-primary/5'
                  }`}
                >
                  <div className="flex justify-between items-center">
                    ...
                    <div className="text-right">
                      <p className={`text-lg font-bold ${row.variance < 0 ? 'text-destructive' : 'text-primary'}`}>
                        {row.variance > 0 ? '+' : ''}{row.variance.toFixed(2)} L
                      </p>
            ...
            {reconciliation.filter((r) => Math.abs(r.variance) > 0.1).length === 0 && (
              <p className="text-center text-muted-foreground py-4">No significant variances detected</p>
            )}
          </div>
        </CardContent>
      </Card>
