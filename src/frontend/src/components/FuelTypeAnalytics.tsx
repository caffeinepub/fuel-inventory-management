...
export default function FuelTypeAnalytics() {
  ...
  const petrolVolumePercent =
    totalVolume > 0 ? (petrolVolume / totalVolume) * 100 : 0;
  const dieselVolumePercent =
    totalVolume > 0 ? (dieselVolume / totalVolume) * 100 : 0;
  const petrolRevenuePercent =
    totalRevenue > 0 ? (petrolRevenue / totalRevenue) * 100 : 0;
  const dieselRevenuePercent =
    totalRevenue > 0 ? (dieselRevenue / totalRevenue) * 100 : 0;
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          Comparative analysis of Petrol vs Diesel performance
        </p>
      </div>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="p-4 border rounded-lg">
              ...
              <p className="text-2xl font-bold">
                {petrolSales.length > 0
                  ? (petrolRevenue / petrolSales.length).toFixed(2)
                  : "0.00"}{" "}
                ₹
              </p>
            </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="p-4 border rounded-lg">
              <p className="text-sm text-muted-foreground mb-1">Average Sale</p>
              <p className="text-2xl font-bold">
                {dieselSales.length > 0
                  ? (dieselRevenue / dieselSales.length).toFixed(2)
                  : "0.00"}{" "}
                ₹
              </p>
            </div>
          ...
          <div className="space-y-6">
            <div>
              ...
              <div className="flex h-8 rounded-lg overflow-hidden">
                ...
                >
                  {petrolVolumePercent > 10 &&
                    `${petrolVolumePercent.toFixed(0)}%`}
                </div>
                <div
                  className="bg-green-600 flex items-center justify-center text-white text-sm font-medium"
                  style={{ width: `${dieselVolumePercent}%` }}
                >
                  {dieselVolumePercent > 10 &&
                    `${dieselVolumePercent.toFixed(0)}%`}
                </div>
              </div>
              <div className="flex justify-between mt-2 text-sm">
                <span className="text-blue-600 font-medium">
                  Petrol: {petrolVolume.toFixed(0)}L
                </span>
                <span className="text-green-600 font-medium">
                  Diesel: {dieselVolume.toFixed(0)}L
                </span>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="flex h-8 rounded-lg overflow-hidden">
                ...
                >
                  {petrolRevenuePercent > 10 &&
                    `${petrolRevenuePercent.toFixed(0)}%`}
                </div>
                <div
                  className="bg-green-600 flex items-center justify-center text-white text-sm font-medium"
                  style={{ width: `${dieselRevenuePercent}%` }}
                >
                  {dieselRevenuePercent > 10 &&
                    `${dieselRevenuePercent.toFixed(0)}%`}
                </div>
              </div>
              <div className="flex justify-between mt-2 text-sm">
                <span className="text-blue-600 font-medium">
                  Petrol: ₹{petrolRevenue.toFixed(0)}
                </span>
                <span className="text-green-600 font-medium">
                  Diesel: ₹{dieselRevenue.toFixed(0)}
                </span>
              </div>
            </div>
          </div>
